...
public class Ch34xSerialDriver implements UsbSerialDriver {
	...
	public class Ch340SerialPort extends CommonUsbSerialPort {
		...
		protected void openInt() throws IOException {
			for (int i = 0; i < mDevice.getInterfaceCount(); i++) {
				...
				if (!mConnection.claimInterface(usbIface, true)) {
					throw new IOException("Não foi possível reivindicar a interface de dados");
				}
			}
		...
		private void checkState(String msg, int request, int value, int[] expected) throws IOException {
			...
			if (ret < 0) {
				throw new IOException("Falha no envio cmd [" + msg + "]");
			}

			if (ret != expected.length) {
				throw new IOException("Esperado " + expected.length + " bytes, mas pegue " + ret + " [" + msg + "]");
			}
		...
		private void setControlLines() throws IOException {
			if (controlOut(0xa4, ~((dtr ? SCL_DTR : 0) | (rts ? SCL_RTS : 0)), 0) < 0) {
				throw new IOException("Falha ao definir linhas de controle");
			}
		}
		...
		private byte getStatus() throws IOException {
			...
			if (ret < 0)
				throw new IOException("Erro ao obter linhas de controle");
			return buffer[0];
		}
		...
		public void setParameters(int baudRate, int dataBits, int stopBits, @Parity int parity) throws IOException {
			...
			switch (dataBits) {
				...
				case DATABITS_8:
					...
					break;
				default:
					throw new IllegalArgumentException("Dados inválidos bits: " + dataBits);
			}
			...
			switch (parity) {
				...
				case PARITY_SPACE:
					...
					break;
				default:
					throw new IllegalArgumentException("Paridade inválida: " + parity);
			}
